...
function build() {
    ...
    ALPINE_VERSION=${2}
    PHP_VERSION=${3}
    docker build --build-arg ALPINE_VERSION=${ALPINE_VERSION} --build-arg PHP_VERSION=${PHP_VERSION}  -t hooklife/php-docker:base-${PHP_VERSION}-${ALPINE_VERSION}-${VERSION} -f Dockerfile.base .
    docker build --build-arg ALPINE_VERSION=${ALPINE_VERSION} --build-arg PHP_VERSION=${PHP_VERSION}  -t hooklife/php-docker:php-fpm-${PHP_VERSION}-alpine-${ALPINE_VERSION}-${VERSION} -f Dockerfile.php-fpm .
    docker build --build-arg ALPINE_VERSION=${ALPINE_VERSION} --build-arg PHP_VERSION=${PHP_VERSION}  -t hooklife/php-docker:php-fpm-nginx-${PHP_VERSION}-alpine-${ALPINE_VERSION}-${VERSION} -f Dockerfile.php-fpm-nginx .
    docker build --build-arg ALPINE_VERSION=${ALPINE_VERSION} --build-arg PHP_VERSION=${PHP_VERSION}  -t hooklife/php-docker:laravel-all-in-one-${PHP_VERSION}-alpine-${ALPINE_VERSION}-${VERSION} -f Dockerfile.laravel-all-in-one .
